...
\bull \.{SET123} \.{\$X,\$Y} is equivalent to \.{OR} \.{\$X,\$Y,0}; it sets
register~X to register~Y. Similarly, \.{SET123} \.{\$X,Y} (when \.Y is
...
\<alias operation>\is\.{SET123}\mid\.{LDA}\cr
...
@#
typedef enum {
@!SET123=0x100,@!IS,@!LOC,@!PREFIX,@!BSPEC,@!ESPEC,@!GREG,@!LOCAL,@/
...
@.YZ field...register number@>
  if (opcode==SET123) val_stack[1].equiv.l<<=8,opcode=0xc1; /* change to \.{OR} */
  ...
  if (opcode==SET123) opcode=0xe3; /* change to \.{SETL} */
  else if (op_bits&immed_bit) opcode++; /* immediate */
